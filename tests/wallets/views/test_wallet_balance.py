...
class TestGet:
    def test_it_returns_active_user_balance(
        ...
        WalletFactory(owner=active_user, amount=Decimal("13.00"))
        WalletFactory(owner=active_user, amount=Decimal("13.00"))
        wallet = WalletFactory(owner=active_user, amount=Decimal("123.00"))

        response = api_client.get(f"/api/wallets/{wallet.pk}/balance/")

        assert response.status_code == 200
        assert response.data["amount"] == "123.00"
    ...
    def test_it_returns_user_balance_if_auth_user_is_admin(
        ...
        WalletFactory(owner=active_user, amount=Decimal("32.00"))
        WalletFactory(owner=active_user, amount=Decimal("44.00"))
        wallet = WalletFactory(owner=active_user, amount=Decimal("1.00"))
